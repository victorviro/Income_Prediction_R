...
# We can also get get the RSS, adjusted R^2 and BIC and Cp

adj_R2 = summary_best_models$adjr2
Cp = summary_best_models$cp
BIC = summary_best_models$bic
number_variables=1:n
dataf= data.frame(number_variables, adj_R2, Cp, BIC )

# We want high values of adjusted R2
plot_adj=ggplot(dataf, aes(x=number_variables, y= adj_R2))+
  geom_line( color='red') + 
  geom_point(size=2.5)+ ggtitle("Plot adjusted R2") +
